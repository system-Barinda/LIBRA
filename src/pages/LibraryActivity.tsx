...
const reservations: ReservationItem[] = [
  ...
  { title: "Threads of Fate", author: 'Milo Sharp', member: 'Milo Sharp', memberId: 'MBR-1170', time: 'Sept 24, 2035 - 02:15 PM', image: '🧣' },
  { title: "My Story", author: 'Julian Cross', member: 'Julian Cross', memberId: 'MBR-1170', time: 'Sept 23, 2035 - 03:15 PM', image: '📘' },
];
...
const StatCard: React.FC<StatCardProps> = ({ title, value, unit, change, isPositive, icon, iconBg }) => (
  <div className="bg-white p-4 lg:p-5 rounded-2xl border border-gray-100 shadow-xs flex items-center justify-between w-full">
    <div className="flex items-center gap-3 lg:gap-4">
      ...
      <div>
        <p className="text-gray-400 text-[11px] lg:text-xs font-medium">{title}</p>
        <h3 className="text-xl lg:text-2xl font-bold text-gray-800 mt-0.5">
          {value} <span className="text-xs lg:text-sm font-normal text-gray-400">{unit}</span>
        </h3>
      </div>
    </div>
    <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-[10px] lg:text-xs font-semibold shrink-0 ${
              ...
              {activities.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50/70 transition-colors">
                  ...
                  <td className="py-4 px-6 font-medium text-gray-700">
                    <div>{row.dueDate}</div>
                    {row.returnDate !== '—' && <div className="text-[10px] text-gray-400 font-normal">Return: {row.returnDate}</div>}
                  </td>
                  <td className="py-4 px-6">
                    {row.status === 'Borrowed' && (
